...
/**
 * Round-trip check for the stage-7b tagged IR (schema 4): slice every fragment's
 * plain text with the span list that is stored next to it and verify that what
 * comes out is what the span claims to be.
 *
 * Schema 4 adds three carriers, and each one gets an invariant rather than a
 * count alone: the structure members' binders are fragments like any other and
 * are walked; the five field-only keys must appear on `label: "field"` members
 * and nowhere else; and `instClass` must be present exactly for `kind:
 * "instance"`. An inherited field additionally may not be named after the
 * structure it appears in -- `getFieldOrigin` returns the parent's projection
 * function, so a child-prefixed name would mean the origin walk was skipped.
 *
 * Copied from `experiments/stage7a/check-spans.ts`, which was copied from
 * `experiments/stage4b/check-spans.ts` and extended with the schema-3
 * `front`/`back` widths (the `splitWhitespaces` run lengths):
 *
...
/** Schema 4. */
let declsWithAttrs = 0;
let attrStrings = 0;
const attrHistogram = new Map<string, number>();
let instances = 0;
let instTypeNames = 0;
let fieldMembers = 0;
let fieldBinders = 0;
let fieldDocs = 0;
let fieldsInherited = 0;
let maxDepth = 0;
...
for (const entry of Deno.readDirSync(`${irDir}/modules`)) {
  ...
  for (const d of m.declarations) {
    ...
    d.members.forEach((mem, i) => {
      checkFragment(`${w}.member[${i}]`, mem.text, mem.code);
      // Schema 4. The five field keys belong to `label: "field"` and to nothing
      // else -- `fieldToHtml` is their only consumer, and a `ctor` or `parent`
      // member carrying them would mean the extractor filled in a default
      // somewhere it should not have.
      const hasFieldKeys = mem.binders !== undefined || mem.doc !== undefined ||
        mem.isDirect !== undefined;
      if (mem.label !== "field") {
        if (hasFieldKeys) fail(w, `member[${i}] is a ${mem.label} but carries field-only keys`);
        return;
      }
      fieldMembers++;
      if (mem.binders === undefined || mem.implicits === undefined || mem.isDirect === undefined) {
        fail(w, `field member[${i}] (${mem.name}) is missing a schema-4 key`);
        return;
      }
      if (mem.implicits.length !== mem.binders.length) {
        fail(w, `field member[${i}] has ${mem.implicits.length} implicit flags for ${mem.binders.length} binders`);
      }
      if (mem.binderCode !== undefined && mem.binderCode.length !== mem.binders.length) {
        fail(w, `field member[${i}] has ${mem.binderCode.length} binder span lists for ${mem.binders.length} binders`);
      }
      fieldBinders += mem.binders.length;
      if (mem.doc != null) fieldDocs++;
      if (!mem.isDirect) {
        fieldsInherited++;
        // `fieldToHtml`'s inherited branch never prints the docstring, and
        // `getFieldOrigin` returns the *parent's* projection function, so the
        // name cannot be prefixed by the structure this member belongs to.
        if (mem.name.startsWith(d.name + ".")) {
          fail(w, `field member[${i}] is inherited but its name ${mem.name} is a child of ${d.name}`);
        }
      }
      mem.binders.forEach((b, j) =>
        checkFragment(`${w}.member[${i}].binder[${j}]`, b, mem.binderCode?.[j])
      );
    });
    // Schema 4: attributes and the instance index.
    if (d.attrs !== undefined) {
      if (d.attrs.length === 0) fail(w, "attrs is present but empty (it should be omitted)");
      declsWithAttrs++;
      attrStrings += d.attrs.length;
      for (const a of d.attrs) attrHistogram.set(a, (attrHistogram.get(a) ?? 0) + 1);
    }
    if ((d.instClass !== undefined) !== (d.kind === "instance")) {
      fail(w, `instClass ${d.instClass === undefined ? "missing" : "present"} for kind ${d.kind}`);
    }
    if (d.instClass !== undefined) {
      instances++;
      instTypeNames += d.instTypes?.length ?? 0;
    }
  }
}
...
console.log(`\nschema 4 — attributes, instance index, structure members`);
console.log(`  declarations with attributes ${declsWithAttrs} carrying ${attrStrings} strings`);
console.log(`  attribute histogram          ${JSON.stringify([...attrHistogram.entries()].sort((a, b) => b[1] - a[1]))}`);
console.log(`  instances with a type index  ${instances}, ${instTypeNames} type names`);
console.log(`  structure fields             ${fieldMembers} (${fieldsInherited} inherited)`);
console.log(`  … binders / docstrings       ${fieldBinders} / ${fieldDocs}`);
if (samples.length > 0) {
