#!/usr/bin/env bash
# Runs the stage-7d extractor (schema 4) over the fixed measurement target and records both
# the timing JSONL and the conditions the numbers were taken under.
#
# usage: run.sh [name] [-- extra args for the binary]
#          name defaults to `stage7d-extract`
#
# Extra args are passed through to the extractor, e.g.
#   run.sh stage7d-noir -- --equations --refs
#   run.sh stage7d-ir  -- --equations --refs --write-ir --ir-dir "$IR_DIR"
#   run.sh stage7d-tag -- --equations --refs --write-ir --tagged-code --ir-dir "$IR_DIR"
#
