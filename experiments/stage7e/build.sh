#!/usr/bin/env bash
# Builds the stage-7e syntax-only parser probe into a native binary under `build/`.
#
# Same shape as the earlier stages: lean-doc has no toolchain and no lakefile of its own,
# so the Lean environment is borrowed from the measurement target through `lake env`.
#
# usage: build.sh
set -euo pipefail
...
"$LAKE" env lean --root="$HERE" \
  -o "$BUILD/Parse.olean" -c "$BUILD/Parse.c" "$HERE/Parse.lean"
...
"$LAKE" env leanc -rdynamic -o "$BUILD/parse" "$BUILD/Parse.c"

echo "built $BUILD/parse"
