/-
Stage 4 preparation experiment for lean-doc (see `docs/approach.md` §5.4 / §5.5 /
§6.1 and `docs/plans/three-axes.md` leg 4).

Started as a copy of `experiments/stage3/Extract.lean` (stages 1-3 are frozen for
reproducibility of their numbers). Everything stage 3 did is still here
unchanged; what stage 4 adds is **IR persistence**: writing the extraction result
out as one JSON file per module, plus a package index and a dependency-side map
slice, and measuring what that costs. `approach.md` §6.1 carried the write cost
as a 仮定 (doc-gen4's 0.68 s); this binary is what replaces it with a measurement.

IR persistence is OFF by default (`--write-ir` turns it on) so that the stage-3
baseline can still be reproduced from this tree.
...
New in stage 4: `--write-ir` persists the result as module-granular IR (see the
...
/-- `none` when `--refs` is off, and then every collector call is a no-op. -/
abbrev RefSink := Option (IO.Ref RefAcc)

/-- Runs `collectConsts` over one formatted fragment and folds the result into the
per-declaration accumulator. -/
def RefSink.collect (sink : RefSink) (fmt : Std.Format)
    (infos : SubExpr.PosMap Elab.Info) : MetaM Unit := do
  let some r := sink | return
...
def ppEquation (refs : RefSink) (e : Expr) : MetaM String :=
  forallTelescope e.consumeMData fun _ body => ppTerm refs body

def computeEquations (refs : RefSink) (v : DefinitionVal) : MetaM (Array String) := do
    ...
    forallTelescopeReducing v.type fun params _ =>
      withLocalDeclD `self (mkAppN (mkConst structName us) params) fun s => do
        ...
        for parent in getStructureParentInfo env structName do
          ...
          acc := acc.push ⟨"parent", parent.projFn, ← ppTerm refs (← inferType proj)⟩
...
def analyze (cfg : Cfg) (module : Name) (name : Name) (ci : ConstantInfo) :
    AnalyzeM (Option DeclOut) := do
  let refs : RefSink ← if cfg.collectRefs then some <$> IO.mkRef {} else pure none
  ...
  return d?.map ({ · with refs := acc.names })
...
text it indexes, which this IR does not carry (see the README). -/
def declToIrJson (d : DeclOut) (refs : Array (Name × Name)) : Json :=
  Json.mkObj [
    ("name", Json.str d.name.toString),
    ("kind", Json.str d.kind),
    ("binders", Json.arr (d.sig.binders.map Json.str)),
...
-/
def writeIRTree (dir : FilePath) (env : Environment) (targets : Array Name)
  ...
  for m in mods do
    ...
    for d in decls do
      ...
      declJson := declJson.push (declToIrJson d pairs)
    let body := Json.mkObj [
      ("schemaVersion", Json.num irSchemaVersion),
      ("module", Json.str m.name.toString),
      ("imports", Json.arr (m.imports.map (Json.str ·.toString))),
  ...
  let index := Json.mkObj [
    ("schemaVersion", Json.num irSchemaVersion),
    ("generator", Json.str "lean-doc/experiments/stage4"),
...
def run (cfg : Cfg) : IO UInt32 := do
  ...
  sink.emit "stage4.initSearchPath" (tSp1 - tSp0)
  ...
  sink.emit "stage4.importModules" (tImp1 - tImp0) [("directImports", toString targets.size)]

  let header := env.header
  sink.emit "stage4.envStats" 0 [("loadedModules", toString header.moduleNames.size)]
  ...
  let env ←
    if cfg.openNamespaces.isEmpty then
      ...
      let (_, st) ← act.toIO { fileName := "<lean-doc/stage4>", fileMap := default }
        { env := env }
      pure st.env
  ...
  let tIdx1 ← IO.monoNanosNow
  sink.emit "stage4.indexLookup" (tIdx1 - tIdx0)
  ...
  sink.emit "stage4.moduleDocs" (tMd1 - tMd0)
  ...
  sink.emit "stage4.tactics" (tTac1 - tTac0)
  ...
  if cfg.tacticsEmulate then
    ...
    sink.emit "stage4.tacticsPerModule" tEmu
  ...
  if cfg.tacticsProbe then
    ...
    let t1 ← IO.monoNanosNow
    probe := some p
    sink.emit "stage4.tacticsProbe" (t1 - t0)
  ...
  sink.emit "stage4.analyze" (tAn1 - tAn0)
  ...
  if cfg.writeIR then
    ...
    irDirUsed := some dir
    let t0 ← IO.monoNanosNow
    irStats ← writeIRTree dir env targets mods results
    let t1 ← IO.monoNanosNow
    sink.emit "stage4.writeIR" (t1 - t0)
  ...
  if let some dumpPath := cfg.dumpPath then
    ...
    sink.emit "stage4.dump" (tD1 - tD0) [("records", toString results.size)]
  ...
  if let some dumpPath := cfg.dumpModulesPath then
    ...
    for m in mods do
      ...
      h.putStr "\n"
    let tD1 ← IO.monoNanosNow
    sink.emit "stage4.dumpModules" (tD1 - tD0) [("records", toString mods.size)]
  ...
  if let some dumpPath := cfg.dumpRefsPath then
    ...
    let tD1 ← IO.monoNanosNow
    sink.emit "stage4.dumpRefs" (tD1 - tD0)
  ...
  sink.emit "stage4.total" (tTotal1 - tTotal0) [("modules", toString targets.size)]
...
def parseArgs (args : List String) : Except String Cfg :=
  ...
  | _ => .error "usage: extract <modules.txt> <out.jsonl> [--equations] [--dump <p>] [--dump-modules <p>] [--only <p>] [--open <ns,..>] [--tag] [--refs] [--dump-refs <p>] [--write-ir] [--ir-dir <p>] [--skip-analyze] [--tactics-emulate] [--tactics-probe]"
where
