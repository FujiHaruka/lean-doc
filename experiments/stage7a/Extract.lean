...
version 3 is this stage's `--tagged-code`, which adds the `splitWhitespaces`
widths to `kind = 1` spans (see `spanToJson`). The flag picks the version rather
than always bumping it, so that with the flag off this binary still reproduces
stage 4's IR byte for byte. -/
def irSchemaVersion (tagged : Bool) : Nat := if tagged then 3 else 1
