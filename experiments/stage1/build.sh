#!/usr/bin/env bash
# Builds the stage-1 extractor into a native binary under `build/`.
#
# lean-doc deliberately has no toolchain, no lakefile and no Mathlib checkout of
# its own. The Lean environment is borrowed from the measurement target through
# `lake env`, which sets LEAN_PATH / DYLD_LIBRARY_PATH and picks the toolchain
# pinned by the target's `lean-toolchain`.
#
# usage: build.sh
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=../../benchmarks/tools/env.sh
source "$HERE/../../benchmarks/tools/env.sh"

LAKE="${LAKE:-$HOME/.elan/bin/lake}"
BUILD="$HERE/build"
mkdir -p "$BUILD"

cd "$TARGET_REPO"

# `--root` is required because the source lives outside the target repository;
# `lean` derives the module name relative to it.
"$LAKE" env lean --root="$HERE" \
  -o "$BUILD/Extract.olean" -c "$BUILD/Extract.c" "$HERE/Extract.lean"

# -rdynamic: `importModules (loadExts := true)` runs module initializers through
# the Lean interpreter, which resolves symbols in the running executable. This is
# what Lake calls `supportInterpreter := true`. Without it the binary dies with
# "Could not find native implementation of external declaration".
"$LAKE" env leanc -rdynamic -o "$BUILD/extract" "$BUILD/Extract.c"

echo "built $BUILD/extract"
