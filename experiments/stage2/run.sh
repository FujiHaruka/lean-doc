#!/usr/bin/env bash
# Runs the stage-2 extractor over the fixed measurement target and records both
# the timing JSONL and the conditions the numbers were taken under.
#
# usage: run.sh [name] [-- extra args for the binary]
#          name defaults to `stage2-extract`
#
# Extra args are passed through to the extractor, e.g.
#   run.sh stage2-eq -- --equations
#   run.sh stage2-probe -- --only probe-names.txt --dump /tmp/probe.jsonl
#
# Writes:
