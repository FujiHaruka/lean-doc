...
#
# Stage 7g's copy of `stage6a/serve-ctl.sh`. Two differences, both deliberate:
#
#   1. `BIN` is the **stage-7d** extractor (IR schema 4, `--jobs N`), not
#      stage 4b's (schema 3). Stage 6a's wiring still points at stage 4b and that
#      is exactly what "本配線" means here: the shipped extractor is 7d's.
#   2. `start` takes a job count and passes `--jobs N` to the server. The server
#      is `run reqCfg (some env)` with `cfg` carried over from the command line
#      (`Extract.lean:2722`), so `--jobs` set at start applies to **every**
#      request. There is no per-request job count, which is why the caller has to
#      start the server with the same N it uses for one-shot extraction if the two
#      are to be compared.
#
...
#
# usage:
#   serve-ctl.sh start   <dir> <modules.txt> [generation-tag] [jobs]
#   serve-ctl.sh request <dir> <modules.txt> <out.jsonl> <ir-dir>
#   serve-ctl.sh stop    <dir>
#   serve-ctl.sh info    <dir>
#
# `JOBS` in the environment is the fallback for the positional job count.
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LD="$(cd "$HERE/../.." && pwd)"
BIN="${EXTRACT_BIN:-$LD/experiments/stage7d/build/extract}"
...
start)
  ...
  [ -x "$BIN" ] || { echo "no extractor at $BIN (run stage7d/build.sh)" >&2; exit 2; }
  mkdir -p "$D"
  ...
  ( cd "$TARGET" && exec "$LAKE" env "$BIN" "$MODULES" "$D/serve-events.jsonl" \
      --equations --refs --write-ir --tagged-code --jobs "$NJOBS" --serve ) \
