...
#
# Stage 7g's copy of `stage5/incremental.sh`. **Three** differences, and nothing
# else — stage 5's copy is untouched because docs quote its numbers:
#
#   (a) extraction goes through `stage7g/extract-once.sh` (the stage-7d
#       extractor, IR schema 4, `--jobs N`) instead of stage 5's, which still
#       runs stage 4b's (schema 3);
#   (b) rendering goes through `stage7d/render.ts` instead of `stage4c/render.ts`;
#   (c) `--serve auto` owns a server for the length of the run.
#
# The intermediate steps (`ledger.ts`, `ownership.ts`, `merge-ir.ts`,
# `prune-pages.ts`, `global.ts`, `impact.ts`) are stage 5's, called in place. One
# implementation is enough; a copy would be a second thing to keep right.
#
...
cleanup () {
  if [ -n "$OWNSERVER" ]; then
    "$HERE/serve-ctl.sh" stop "$OWNSERVER" >> "$WORK/serve.log" 2>&1 || true
    OWNSERVER=""
  fi
}
...
if [ -n "$SERVE" ]; then
  ...
  A=$(now)
  JOBS="$JOBS" "$HERE/serve-ctl.sh" start "$OWNSERVER" "$MODULES" per-run "$JOBS" \
    > "$WORK/serve.log" 2>&1
...
deno run --allow-read --allow-write --allow-env "$S5/ledger.ts" check \
...
while [ "$(grep -c . "$ROUND_IN" || true)" -gt 0 ] || \
  ...
  if [ "$L31" = on ]; then
    A=$(now)
    deno run --allow-read --allow-write "$S5/ownership.ts" --base "$IR" \
  ...
  A=$(now)
  deno run --allow-read --allow-write "$S5/merge-ir.ts" --base "$IR" \
...
if [ "${NREMOVED:-0}" -gt 0 ]; then
  deno run --allow-read --allow-write "$S5/prune-pages.ts" --pages "$PAGES" \
...
: > "$GLOBALSET"
if [ -f "$MAPBEFORE" ]; then
  deno run --allow-read --allow-write "$S5/global.ts" delta --before "$MAPBEFORE" \
...
deno run --allow-read --allow-write "$S5/impact.ts" --ir "$IR" \
...
if [ "${#ONLY[@]}" -eq 0 ]; then
  ...
  deno run --allow-read --allow-write "$RENDER_TS" \
