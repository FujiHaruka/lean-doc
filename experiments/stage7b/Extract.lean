/-
Stage 7b experiment for lean-doc: a copy of `experiments/stage7a/Extract.lean`
(schema 3) that adds the three things `approach.md` §6.1 named as missing —
attribute collection, the instance type index, and the structure members'
binders / docstring / origin — and bumps the tagged IR to **schema 4**.

Stage 7a is left untouched: its 96.506% / 15.24 MiB / 13.7084 s are committed
numbers that `docs/approach.md` quotes, and an extractor that has been edited
cannot reproduce them.

Everything below this paragraph is stage 4b's original header, still accurate
except where the stage-7a and stage-7b notes above and in `irSchemaVersion` say
otherwise.
...
def run (cfg : Cfg) (preEnv : Option Environment := none) : IO UInt32 := do
  ...
  for (name, module) in (if cfg.skipAnalyze then #[] else candidates) do
    ...
    counters := {
      ...
      attrNanos := counters.attrNanos + c.attrNanos,
      attrCount := counters.attrCount + c.attrCount,
      attrDecls := counters.attrDecls + c.attrDecls,
      instNanos := counters.instNanos + c.instNanos,
      instCount := counters.instCount + c.instCount,
      instTypeNames := counters.instTypeNames + c.instTypeNames,
      memberNanos := counters.memberNanos + c.memberNanos,
      memberFields := counters.memberFields + c.memberFields,
      memberInherited := counters.memberInherited + c.memberInherited }
    match outcome with
