...
//
// Stage 7b: rebuild doc-gen4's HTML from the **schema-4** IR without starting
// Lean. Copied from `experiments/stage7a/render.ts` (which reads schema 3) and
// changed in three places, all of them things the IR could not say before:
//
//   * `div.attributes` -- the `@[...]` line (`Output/Module.lean:88-94`);
//   * `fieldToHtml`'s other half -- the field's own binders, its docstring and
//     its origin (`inherited_field`);
//   * nothing at all for the instance type index, which schema 4 carries and no
//     module page prints. The browser builds those lists from
//     `declaration-data.bmp`; see `approach.md` §5.5 L3-3.
//
// Stage 7a is left as it was, because its 96.506% and its timings are the
// committed schema-3 baseline, and stage 4c before it the schema-2 one.
//
