/-
Stage 3 verification experiment for lean-doc (see `docs/approach.md` §7 and
`docs/plans/stage3.md`).

Started as a copy of `experiments/stage2/Extract.lean` (stage 1 and stage 2 are
frozen for reproducibility of their numbers). Everything stage 2 did is still
here unchanged; what stage 3 adds is increment 1 of its plan: the **constants a
signature refers to**, which is what doc-gen4 turns into hyperlinks. Those names
are the demand side of the "declaration name -> module -> URL" map, i.e. the
lower bound on how big that map has to be.
...
New in stage 3: `--refs` collects, per declaration, every constant doc-gen4's
...
def run (cfg : Cfg) : IO UInt32 := do
  ...
  sink.emit "stage3.initSearchPath" (tSp1 - tSp0)
  ...
  sink.emit "stage3.importModules" (tImp1 - tImp0) [("directImports", toString targets.size)]

  let header := env.header
  sink.emit "stage3.envStats" 0 [("loadedModules", toString header.moduleNames.size)]
  ...
  let env ←
    if cfg.openNamespaces.isEmpty then
      ...
      let (_, st) ← act.toIO { fileName := "<lean-doc/stage3>", fileMap := default }
        { env := env }
      pure st.env
  ...
  sink.emit "stage3.indexLookup" (tIdx1 - tIdx0)
  ...
  sink.emit "stage3.moduleDocs" (tMd1 - tMd0)
  ...
  sink.emit "stage3.tactics" (tTac1 - tTac0)
  ...
  if cfg.tacticsEmulate then
    ...
    sink.emit "stage3.tacticsPerModule" tEmu
  ...
  if cfg.tacticsProbe then
    ...
    let t1 ← IO.monoNanosNow
    probe := some p
    sink.emit "stage3.tacticsProbe" (t1 - t0)
  ...
  sink.emit "stage3.analyze" (tAn1 - tAn0)
  ...
  if let some dumpPath := cfg.dumpPath then
    ...
    sink.emit "stage3.dump" (tD1 - tD0) [("records", toString results.size)]
  ...
  if let some dumpPath := cfg.dumpModulesPath then
    ...
    for m in mods do
      ...
      h.putStr "\n"
    let tD1 ← IO.monoNanosNow
    sink.emit "stage3.dumpModules" (tD1 - tD0) [("records", toString mods.size)]

  -- The unique set of referenced constants: the demand side of the link map
  -- (`docs/plans/stage3.md` §3, increment 1). One line per constant, in order of
  ...
  if let some dumpPath := cfg.dumpRefsPath then
    ...
    let tD1 ← IO.monoNanosNow
    sink.emit "stage3.dumpRefs" (tD1 - tD0)
  ...
  sink.emit "stage3.total" (tTotal1 - tTotal0) [("modules", toString targets.size)]
...
def parseArgs (args : List String) : Except String Cfg :=
  ...
  | _ => .error "usage: extract <modules.txt> <out.jsonl> [--equations] [--dump <p>] [--dump-modules <p>] [--only <p>] [--open <ns,..>] [--tag] [--refs] [--dump-refs <p>] [--skip-analyze] [--tactics-emulate] [--tactics-probe]"
where
