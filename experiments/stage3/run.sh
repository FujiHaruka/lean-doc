#!/usr/bin/env bash
# Runs the stage-3 extractor over the fixed measurement target and records both
# the timing JSONL and the conditions the numbers were taken under.
#
# usage: run.sh [name] [-- extra args for the binary]
#          name defaults to `stage3-extract`
#
# Extra args are passed through to the extractor, e.g.
#   run.sh stage3-eq -- --equations
#   run.sh stage3-refs -- --refs --dump-refs "$PWD/refs.jsonl"
#   run.sh stage3-probe -- --only probe-names.txt --dump /tmp/probe.jsonl
#
