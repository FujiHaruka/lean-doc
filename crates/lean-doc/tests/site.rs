//! `lean-doc site` — full generation in one command (milestone M3-d1).
//!
...
fn help_is_answered_and_the_usage_names_the_subcommand() {
    for args in [vec!["site", "--help"], vec!["--help"]] {
        ...
        assert!(text.contains("lean-doc site"), "{args:?}: {text}");
...
/// shape `lean-doc-global/tests/state_and_delta.rs` writes.
...
fn synthetic_ir(root: &Path) {
    ...
    let index = json!({
        ...
        "generator": "lean-doc/crates/lean-doc/tests/site.rs",
...
/// A directory that removes itself, as in `lean-doc-global/tests/global.rs`.
struct TempDir {
    path: PathBuf,
}
