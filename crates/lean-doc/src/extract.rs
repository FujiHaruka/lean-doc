//! `lean-doc extract` — one extractor process over a module list, and its phase
...
//!
//! - **`lean-doc incremental --extractor` already names a program**, and its
...
//!   `--extractor <lean-doc> --extractor-arg extract` — **without closing the
    ...
    let Some(timings) = timings else {
        return usage(
            "--timings <file> is required: it is the extractor's phase timers folded into one \
             JSON object, and `lean-doc incremental` merges it into the run's record",
        );
    };
    if jobs == 0 {
    ...
    let Some(target) = target else {
        return usage(
            "--target <repo> is required (or TARGET_REPO): the Lean package being documented. \
             ...
             search path without lean-doc owning a toolchain",
        );
    };
