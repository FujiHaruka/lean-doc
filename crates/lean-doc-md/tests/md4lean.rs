...
use lean_doc_md::{AttrText, Block, Document, Li, Text};
...
///
/// The docstring cases go through [`lean_doc_md::parse`] rather than through
...
fn check(case: &Case, docstring_flags: u32) -> Option<String> {
    let got = if case.flags == docstring_flags {
        lean_doc_md::parse(&case.md)
    } else {
        lean_doc_md::parse_with_flags(&case.md, case.flags)
    };
    match (&case.ast, got) {
...
fn the_fixture_is_md4leans_own_output() {
    ...
    assert_eq!(e.flags, lean_doc_md::DOCSTRING_FLAGS);
...
/// Skips otherwise, exactly as `lean-doc-ir`'s IR fixture does: `cargo test`
/// has to pass where the target package does not exist.
#[test]
fn the_whole_corpus() {
    let path = std::env::var("LEAN_DOC_MD4LEAN_FULL").unwrap_or_else(|_| DEFAULT_FULL.to_owned());
    if !std::path::Path::new(&path).is_file() {
        eprintln!(
            "skipping: no --full file at {path} (set LEAN_DOC_MD4LEAN_FULL, or regenerate with \
             tests/oracle/gen-md4lean-expected.ts --full)"
        );
        return;
    }
...
fn the_inputs_that_kill_md4lean_parse_here() {
    ...
    for crasher in &crashers {
        match lean_doc_md::parse_with_flags(&crasher.md, crasher.flags) {
            ...
            Err(lean_doc_md::Error::Unrepresentable(_)) => {
...
fn a_nul_in_ordinary_text_is_a_normal_case() {
    let doc = lean_doc_md::parse("a\0b\n").expect("a NUL in text");
    assert_eq!(
        doc.blocks,
        vec![Block::P(vec![
