...
//! completed by hand before it can be opened is not a tree `lean-doc build`
//! finished.
//!
...
//! `lean_doc_incr::render_key`). **A page's bytes do not depend on these
...
/// `lean_doc_global::ARTIFACT_PATHS` spells `declarations/name-map.json`.
///
/// Shaped after `lean_doc_global::Artifacts::files`: one array of
...
mod tests {
    ...
    ///
    /// The four files are the ones that emit markup. `lean-doc-md` also emits
