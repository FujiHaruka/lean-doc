...
//! **no entry at all**: `lean-doc build` wrote 432 module pages and no
//! `index.html`, while every page's top bar linked to `index.html` and
//! `search.html` and every `Sort` in every signature linked to
//! `foundational_types.html` (`lean_doc_render::code`). Three dead links on all
...
//! mistake
//!
//! It is not one. What decides a page's shape is [`lean_doc_render`] — the head,
...
use lean_doc_render::{SiteMeta, break_within, head_html, topbar_html};
