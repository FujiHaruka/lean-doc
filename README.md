...
**現在: v0.1 + M8。** ゲート B (第 2 の対象で 1 コマンドが通る) は通っている
【最新の実走は 2026-08-17、`tools/target2-gate.sh all` = all checks passed →
[`benchmarks/results/g3-gate-b-2026-08-17.txt`](benchmarks/results/g3-gate-b-2026-08-17.txt)】。
...
| **既存モジュールに 1 宣言足す + `lake build`** (1 抽出 / **1 ページ**) | warm **約 4.6 s** (壁時計) | 【実測 2026-08-17】**うち約 2.4 s は Lean の環境ロード** (既知の床)。同じ編集は 2026-08-17 の 3 段の前には **422 ページ全部を再描画**していて、内側時計で **6.24 s** だった (壁時計はその条件では測っていない) → [`docs/plans/reextract-count.md`](docs/plans/reextract-count.md) §6 |
...
10. **プレビューモードと、検索の依存横断** は**持たない**と決めている
    (→ [`docs/approach.md`](docs/approach.md) §9)。**テーマは M8 で持った** — 検索も
    自パッケージの 4,750 宣言については持つ。**依存パッケージの宣言は検索できない。**
...
14. **等幅フォントのフォールバック (M8 決定 2)** — Web フォントを読まないので、本文に出る
...
15. **数式は組版しない (M8 決定 3)** — 対象 348 ページ中 TeX があるのは **1 ページ、`$` 12 個**
    【実測】なので、MathJax も KaTeX も積んでいない。`$$…$$` は**文字列として読めるだけ**。
    数式の多いパッケージでは判断が変わる。
16. **`file://` で開くとモジュールツリーが出ない (M8 決定 4)** — `fetch` が同一オリジンで止まる。
    **捨てた**のであって気づいていないのではない。本文は読めるし、`<noscript>` が `index.html`
    へ導く。配布先は GitHub Pages (HTTP) を前提にしている。
17. **実ブラウザでの描画は確認していない。** 検証したのは「生成された HTML の構造」と
    「emit する class が全部 CSS にある」ことだけ (`assets.rs` のテスト)。
    **375 px の見え方・フォントのフォールバック・ダークモードの実際の色・スクリーンリーダーは
    どれも未確認。** CSS を読んで正しいはずだ、以上のことは言えない。
