...
**済んだことにしない。** ここに書いてあるものは v0.1 では測っていない / 通していない。**13 件**。
**下の[「意図的に持たないもの」3 件](#意図的に持たないもの--未検証ではなく決定)とは分けてある** —
あちらは測っていないのではなく、持たないと決めたもの。

**2026-08-17 に 2 件潰した**。どちらも**「未検証」ではなく壊れていた**:
**版固定できない依存へのリンク** (相対リンクが 404 していた → [`e2e/README.md`](e2e/README.md)) と、
**実在の公開パッケージでの実走** (`batteries` で 3 件出た →
[`benchmarks/results/batteries-2026-08-17.txt`](benchmarks/results/batteries-2026-08-17.txt))。
**測っていないものは、測るまで何も言えない**という主張がそのまま実演された形なので、
残りの 13 件を「たぶん大丈夫」と読まないこと。

1. **GitHub Actions 実走** — **テンプレートの手順は実走して緑になった**
...
2. **`lake exe cache get`** — **実走した** (同 run。`--cache-get` を渡した経路も通っている)。
   ただし**ローカルの計測では依存が既にあるので通っていない**経路のまま。
3. **`«…»` を含む依存側モジュールの `.lidx` 綴り差** — **綴り差は実在し、解決も実際に外れた**
...
4. **「同名宣言を複数モジュールが持つ形」** — 対象 1 には **25 件**ある
   ...
   **`batteries` でも当たらなかった**【実測 2026-08-17】 — search-index の **3,030 名が
   すべて distinct**、重複 0。
5. **CI ランナーが cold 側かどうか** — **測った。cold 側ではなかった**【実測 2026-08-16、
   ...
   **`batteries` はここに当たらない**【実測 2026-08-17】 — 176 モジュールで対象 1 (432) より小さい。
6. **Lean のバージョン差** — 動作を確認したのは **v4.31.0 のみ**
   (Mathlib `fabf563a7c95`)。あらゆるバージョンへの後方互換は**意図的にやらない**。
7. **fenced code block 中の NUL** — MD4Lean が SIGSEGV する入力なので**合わせる相手が
   存在しない**。lean-doc は落ちないほうに倒しているが、出力には U+FFFD に加えて
   **生の NUL が 1 バイト残る**【実測】。
8. **プロトタイプ (TS) との登録済みの乖離 3 件** (**プロトタイプは HEAD に無い** —
   ...
   **`batteries` では heading id が 69 ファイルに出た**【実測 2026-08-17】ので、
   「対象 1 に出現しない」という根拠はこの対象では成立しない。**それでも未検証のまま**なのは、
   **照合相手が無い**から (doc-gen4 が書いた batteries の出力を持っていない)。
   詳細と母数は [`docs/implementation-plan.md`](docs/implementation-plan.md) §5。
9. **依存 root の 39 件中 27 件にオフラインのオラクルが無い**【実測 →
...
10. **`--root` 無しの `ledger check` は `build` と違う `renderKey` を出す** — 依存リンクの
    写像が key に入ったため。**全ページ再生成になるだけで誤りは出ない**が、手で段を回す人は踏む。
11. **等幅フォントのフォールバック (M8 決定 2)** — **2 OS で測った。字形は 1 つも欠けなかった**
...
12. **実ブラウザで見ていないのは「見た目そのもの」。** **動作は見ている** —
    ...
    **スクリーンリーダー**。CSS を読んで正しいはずだ、以上のことが言えないのはこの 2 つに縮んだ
    — **フォントの字形は 2026-08-17 に検査 8 として入り、上の 11 が持つ**。
13. **md4c FFI に残っているのは「リークを見ていない」と「1 機材でしか探していない」**。
...
**上の 13 件と混ぜない。** これは測っていないのではなく、**持たないと決めた**もの。
