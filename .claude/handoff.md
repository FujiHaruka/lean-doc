...
## 残っている判断 2 件 (どちらもユーザー判断待ち)

1. **`.lidx` の綴りを解決すべきか** (R3 が出した仕様判断)。
   docstring が依存モジュールを 3 通りに綴ると **2 つは解決し 1 つ (`Dep-Aux.Basic`) はしない**。
   「これはどのモジュールか」に答える経路が 2 本あり、unescape するかで答えが違う。
   **`tools/pinned-dep-gate.sh` が現状を pin している**ので、変えるならゲートも同じコミットで動く。
   → `docs/plans/residual-sweep.md` R3 の「残った判断」
2. **未検証は 1 点に狭まった** — 「**小 RAM の Linux ランナーでの実走**」。
   算術 (3.3 + olean 5.7 = 9.0 GB > 7 GB) は出ているが、実走はしていない。
   R5 (他人のリポジトリから `uses:`) は**保留のまま**で、これは未検証項目としては別枠。

## Next step

**決まっていない。** R1〜R4 は閉じ、R5 は保留の判断が付いている。候補:

- 上の判断 1 (`.lidx` の綴り) を決める → 決めたらゲートと実装を同じコミットで動かす
