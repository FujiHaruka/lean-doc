...
| **M4** | 抽出器を製品ツリーへ + **常駐の自動起動・停止**を Rust から配線 + CLI の形を確定 | **1 コマンド**で対象リポジトリのサイトが出る。**M4-a (`extractor/` へ移動) / M4-b (`lean-doc extract`) / M4-c (常駐) は通過済み** → §7。残りは **M4-d** (`build` 1 コマンド + 台帳の書き戻し + `--lib` の出所) |
