...
製品ツリー `crates/` は M0 で作り、M1 / M2 / M3-a / M3-b / M3-c 分が入っている (→ §7)。`experiments/` は数字の
...
`stage5/impact.ts` (231) と `stage5/prune-pages.ts` (138) → M3-c、
`stage7h/run.sh` の `render()` 部 (3 行) → M3-d1、`stage7h/incremental.sh` (441) と
`run.sh` の `modlist()` (5 行) → M3-d2。
...
**`--modules` は M3-b では再現しなかった**【実測】 — `merge-ir.ts:29,40` が usage に出しているが
`opt("--modules")` を一度も読んでおらず、**実装されていないフラグ**だったため。**M3-d2b で理由付きで足した**
(→ M3-d2b の節)。プロトタイプに無い挙動なので、移設ではなくこちらの設計として書いてある。
**ラウンドと `--max-rounds` / exit 5 は M3-d に残す** —
...
移設元 `stage5/{impact.ts,prune-pages.ts}` (231 + 138)、移設先 `lean-doc-incr/src/{impact,prune}.rs`、
CLI は `lean-doc impact|prune`。ハーネス `tools/impact-{reference,compare}.sh` は**シナリオ 29 本 (impact 18 /
prune 11) を 1 箇所で定義**して両実装に回す。**比較器に例外リストは無い**。

**母数 3,631 / 一致 3,631 / 差分 0 / 欠落 0**。**母数の内訳を出す** — この 3,631 は 1 つの問いの答えではない:

| | 数 | 何 |
|---|---:|---|
| 生き残ったページ | **3,458** | `prune` を回した 8 本のページ木の**残存ファイル**。「どちらも消さなかった」の確認 |
| 計算された記録 | **165** | 終了コード / stdout / `--print-set` の有無 / 選択集合 / summary / census / 残存一覧 |
| 入力 fixture | 8 | 両実装に同じものを食わせた証拠 |
| 比較対象外 | 29 | `*-stderr.txt` (診断文言は実装のもの。「文句を言ったか」だけ全件比較) |

**「同じ答えを計算するか」の分母は 165、「正しいファイルだけを消したか」は 3,458。**

**分岐は 69 中 16 が実データに到達しない**【実測】。**mutation 13 件のうち全件比較で捕まるのは 7 件**。
逃げる 6 件は U1 のソート / `selectedDeclarations` の出所 / `selectedIrBytes` の重複除去 / 空パスの symlink /
`orphanPages` の打ち切り 20→50 / **`metadata` → `symlink_metadata`**。**最後の 1 件はテストでも捕まらなかった**
(本物の穴。ぶら下がりシンボリックリンクだけが両者を分ける。分岐 `pageIsDanglingSymlink` と手書きケースで塞いだ)。
...
出力先に。**独立したスクリプトは存在しない**)。移設先は `lean-doc site`、ハーネスは `tools/site-compare.sh`。

**母数 438 / 一致 437 / 差分 1 / 欠落 0 / 余分 0。差分 1 は §5 の登録済み乖離のみ**
(`…TimeBandLimiting/Count.html`)。**比較器に例外リストは無い**。
**合成が何も足していないことを別に確かめた** — `render` + `global` を別プロセスで回した木と **438/438 一致**、
M2 ゲートが 2 コマンドで作った木とも **438/438 一致**。

**決定 4 を既定ではなくフラグの形で塞いだ**【判断】 — `--link-index` / `--no-link-index` の**どちらか必須**で、
拒否文言は `render` と定数を共有する (2 箇所が別々に腐らないように)。**ガードが効いていることを実測した** —
同じ IR・同じ `--source-url` で `.lidx` の有無だけを振ると **438 中 150 が差分** (= 432 ページ中 150。
全域 6 本は不動)。**移設元はこれを渡していない**ので、そのまま写せば 3 度目を踏んでいた。
**受け付けないフラグは「unknown argument」にせず名指しで断る**【判断】 — `--only` / `--only-from` /
`--before` / `--print-set` / `--delta-json` はどれも `site` が呼ぶ側の実在フラグなので、
呼び手に要る答えは「綴り間違い」ではなく「なぜここに無いか」。

**フル生成と増分で 2 段の順序が逆なのは矛盾ではない**【実測】 — `run.sh` は render → global、
`incremental.sh` は global (step 6) → render (step 7)。制約 2 は**全域 delta が再生成集合の半分だから**で
あって成果物の依存ではない。フル生成には delta が無いので順序は自由 (両段の書き込みは 1 ファイルも衝突しない)。
**M3-d2 がこれを「render が先」と読まないよう `site()` の docstring に明記した。**

### M3-d2 の結果 — `incremental` + `modules` (並べる人)【すべて実測 2026-08-12】

移設元 `stage7h/incremental.sh` (441) + `run.sh:82-86` の `modlist()`、移設先
`crates/lean-doc/src/pipeline.rs` (bin crate。render + global + incr の全部に依存するのはここだけ)、
CLI は `lean-doc incremental|modules`。**段はすべてライブラリ呼び出しで、外部プロセスは抽出器だけ**。

**抽出器の継ぎ目** — `--extractor <program>` は**必須で既定値なし**【判断】。起動形は
`<program> [<extractor-arg>…] --modules <round-in> --ir-dir <dir> --timings <file>` =
`stage7g/extract-once.sh:24,47` の必須引数そのもの。役目は 2 つ: ①**M4 の境界** (既定値を持たせると
製品が凍結済み `experiments/` に依存する)、②**Lean 無しでパイプラインをテストできる形** (テストは
焼いた部分 IR 木を配る偽抽出器を渡す。これが無いと全テストが Lean 依存になり事実上テストされない)。
`--jobs` は受け付けない (§6 制約 6。`--extractor-arg --jobs --extractor-arg 4` で通る)。

**ゲート = `oracle.sh` の 7 状態をパイプライン全体に対して回し、各状態でフル生成 (`lean-doc site`) と
byte 比較**【実測】。合成パッケージ (6〜7 モジュール) を source / olean / IR の 3 層で持ち、
olean と IR を**独立に**動かせる形にした (宣言の移動で referrer の olean が動かない = L3-1 が要る理由を
fixture が含むため)。**サイトは 7 状態すべて全件一致** (12/12・12/12・12/12・11/11・12/12・12/12・12/12)。
**IR は M3-d2 の時点で 55 中 52 一致**【実測】 — 3 状態 (added / restored / stale-state) が `index.json`
だけ差分だった。`merge` が base index に無いモジュールを**末尾に push** し、from-scratch 抽出は
モジュール一覧の順に並べるので、**モジュールが増えた木は同じエントリを違う順で持つ**。
**サイトのバイトには届かない** (7 状態すべてで site 一致) ので気づかれない形の乖離。
M3-b の 436/436 はモジュールの増減が無い合流だったので、これは M3-d2 で初めて見えた。
→ **M3-d2b で塞いだ (55/55)**。下記。
...
照合し、実測で**同じ 432 件・同じ集合だが 163 行が別の位置に来る** (`LC_ALL=C` = code point 順と比べて。
`diff` のハンク数 22 はブロック移動を 1 と数えるので**移動量を大幅に過小評価する** — 分母は「同じ添字で
食い違う行数 / 432」で取ること)。原因はロケール照合が `.` と大小文字を無視すること
(`Shannon.AWGN.Achievability` ↔ `Shannon.ArithmeticCoding`)。**U1 の UTF-16 順**に固定した
(`LC_ALL=C sort` と 432/432 一致)。ロケール依存の順序は機械が変われば再現しない。

**この順序は 2 つの成果物のバイトを作る**【実測 2026-08-12】 — ①**台帳の `modules` 配列順**
(同じ 432 モジュールで 2 通りの順のリストから台帳を作ると **120,103 B 同士でバイトが違う**)、
②**IR の `index.json` の `modules` 配列順** — **抽出器は渡されたリスト順をそのまま保つ**
(`w7h/base-ir/index.json` はプロトタイプのロケール順と完全一致、code point 順とは 163 行違う)。
**サイトのバイトには届かない** (`check` は再抽出集合を、`impact` は選択をそれぞれソートする)。
→ **M3-d4 のゲートは増分側と from-scratch 側に必ず同じモジュール一覧を渡すこと。**
違うリストを渡すと、パイプラインとは無関係に `index.json` だけで落ちる。**`--ledger` は読むだけで書き戻さない** —
プロトタイプも同じ (`run.sh:167` が毎回 seed し直す)。連続実行のあいだ誰が台帳を更新するかは **M4**。

**分岐は 59 中 27 が 7 状態に到達しない**【実測】。**mutation 14 件のうちゲート (7 状態の byte 比較のみ)
が捕まえるのは 5 件**、手書きケースが 7 件、**逃げた 2 件はどちらも等価変異**:
`--exclude` を落としても 2 巡目は lost/gained が空 (2 巡目に来るモジュールは olean が動いていないので
宣言名が base と同じ) で誰も報告できず、削除リストを毎ラウンド渡しても両段が「base index に居ないもの」を
落とす。**どちらも「捕まらないこと」を assert で固定**した (3 巡目が可能になったら落ちる)。

### M3-d2b の結果 — `merge --modules` (index の並び)【すべて実測 2026-08-15】

M3-d2 が残した「モジュールが増えると `index.json` だけ from-scratch と違う」を塞いだ。
**`MergeOptions.modules` / `lean-doc merge --modules <file>` を新設**し、パイプラインは `detect` に渡すのと
**同じ一覧**を `merge` にも渡す。渡さないときの挙動は不変 (base 順 + 末尾 push)。
**プロトタイプが usage に出して一度も読まないフラグ** (`merge-ir.ts:29,40`) なので、これは移設ではなく
**こちらの設計**。前提は実測済み — **抽出器は渡されたリスト順をそのまま保つ**
(`w7h/base-ir/index.json` の 432 件はロケール `sort` の出力と 432/432 一致)。

- **7 状態オラクル: IR 55/55 バイト一致** (状態別 8・8・8・7・8・8・8。**前は 52/55**)。**サイトは 83/83 のまま**。
  差分集合の固定は消さず**空であることを assert** にしたので、戻ったら状態名が出る。
- **M3-b の 436/436 は不動**【実測】 — 何も変わらない再抽出を合流した木は from-scratch 木と
  `/usr/bin/diff -r` で差分 0。`--modules` を渡さない経路は 1 バイトも動いていない。
- **リストと木が食い違ったら exit 3 で拒否**【判断】 — 黙って末尾に足せば増分と from-scratch が
  永久にずれ、黙って落とせばファイルがあるのに index に居ないモジュールができる。どちらも
  `index.json` だけの差なので**ページのバイトに出ず、ゲートで見えない**。**書き出す前に**判定するので、
  拒否された木はそのまま作り直せる。
- **並びは `deps/*.json` のバイトにも届く**【実測】 — 依存名は「後に来たモジュールが勝つ」ので、
  同じ名前を別の定義モジュール経由で参照する 2 モジュールの順序が変わるとスライスのバイトが変わる。
  手書きケースで両方の順の出力を並べて固定した。
- **分岐は 71 中 23 が実データに到達しない** (M3-d2b で +6、うち 5 件は手書きケースだけが到達)。
  **mutation 6 件のうち 7 状態のバイト比較が捕まえるのは 2 件** (`--modules` を使わない / パイプラインが
  渡さない)。逃げた 4 件 (食い違いを黙って飲む / 片方向しか見ない / 重複を潰さない / 並べ替えを
  deps の後に置く) は**すべて手書きケースが捕まえる**。7 状態の world には重複も依存名の衝突も無い。
