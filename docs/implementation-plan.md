...
| **M3** | **増分 4 本 + パイプライン**を Rust へ | 本物の移動と本物の削除を `lake build` ごと回して**フルビルドと byte 一致**。**M3-a (`detect`) / M3-b (`ownership` + `merge`) / M3-c (`impact` + `prune`) は通過済み** → §7。残りは **M3-d** (パイプライン + ゲート) |
...
### M3-d が引き取る負債【すべて実測 2026-08-12。M3-c で判明】
