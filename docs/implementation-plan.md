...
- **README「未検証項目」15 件**【2026-08-17 に棚卸しした。締めた日は 18 件と書いていたが、
  うち 3 件は「持たないと決めた」もので測っていないのではなく、README で別節に分けた。
  同時に **1 件が古かった** — 「実ブラウザ未確認」は `tools/browser-gate.sh` が CI で
  9 検査を回している (Q8 で UI-3 決着) 以上、**残っているのはフォント字形 / ダークの実際の色 /
  スクリーンリーダーの 3 つ**に縮んでいた】。特に **実在の公開パッケージでの実走**
