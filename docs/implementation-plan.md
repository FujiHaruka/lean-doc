...
- **README「未検証項目」14 件**【締めた日は 18 件。2026-08-17 の棚卸しで **18 → 15 件**に
  ...
  15 → 14 件** — こちらは**実際に潰した 1 件**で、#10 (版固定できない依存のフォールバック) は
  **未検証ではなく壊れていた** (→ §M7「訂正 — 版固定できない依存のフォールバックは
...
#### 訂正 — 版固定できない依存のフォールバックは死にリンクだった【実測 2026-08-17】

**M7-c が「相対ページリンクへフォールバックする」と書いた枝は、依存に対しては壊れていた。**
`ExternalLinks::href` はマップに root が無いモジュールを**自パッケージのもの**とみなして
相対リンクを書くが、**版固定できない依存も同じ枝に落ちる** — `lean-doc` は依存にページを
書かないので、**このサイトが決して書かないページへのリンク**になる。
...
直した形: `ExternalLinks` が**空 base の root** を持てるようにし、`href` を `Option<String>` に。
**版固定できない依存にはリンクを張らず、名前をテキストで残す**。根拠は `autolink.rs` が既に
書いている原則「A link to the wrong page is worse than no link」で、
