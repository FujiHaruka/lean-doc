...
| **M3** | **増分 4 本 + パイプライン**を Rust へ | 本物の移動と本物の削除を `lake build` ごと回して**フルビルドと byte 一致**。**M3-a (`detect`) / M3-b (`ownership` + `merge`) / M3-c (`impact` + `prune`) / M3-d1 (`site` = フル生成) は通過済み** → §7。残りは **M3-d2** (増分パイプライン) と **M3-d3** (クローンでのゲート) |
...
`stage7h/run.sh` の `render()` 部 (3 行) → M3-d1。

| 未移設のパス | 行 | 役割 | M |
|---|---:|---|---|
| `stage7h/incremental.sh` | 441 | 増分パイプライン本体 (7 段) | **M3-d2** |
...
### M3-d が引き取る負債【1〜5 は実測 2026-08-12 / M3-c、6〜8 は M3-d1】

**移設ではなく結線の問題**なので、`impact` / `prune` には入れず M3-d の責任として登録する:

1. **`incremental.sh:354-360` はプロトタイプ側の穴** — changed 集合が空だと `impact.ts:179` がブロックごと
   飛ばして `impact-set.txt` を**書かない**。`sort -u "$WORK/impact-set.txt" "$GLOBALSET"` は存在しない
   ファイルで失敗し、**レンダー集合が空に落ちて global 由来の再生成集合が黙って捨てられる**。
   実データでは「IR 不変 ⇒ 全域 delta も空」なので到達しないが、**そのまま写すと仮定ごと持ち込む**
2. **0 B のファイルと存在しないファイルは別物** — `global` の `--print-set` は変化ゼロで **0 B** を書き、
   `impact` の `--print-set` は選択ゼロで**書かない**。M3-d は「無いファイル = 空集合」として扱う必要がある
   (§5 の「空集合と未指定を型で区別する」と対)
3. **`global-set.txt` と `impact-set.txt` の合流は M3-d の責任** — `impact` は全域 delta を入力に取らない
4. **`prune --ir` をサイトに向けると全域成果物のうち `.html` 3 本 (`navbar` / `references` / `tactics`) が消える**
   【実測。438 → 435】 — 孤児判定が「生きたモジュールページでない `.html` 全部」だから。プロトタイプの
   パイプラインは `--ir` を渡していない (`incremental.sh:304-305`) ので今まで誰も踏んでいない
5. **モジュール一覧の glob** (§5 の M3-d 項) — `impact` は `index.json` 由来で `.lake/build` を見ない
6. **`name-map.json` は「before の写像」であると同時に `global` が in-place で上書きする先**
   【実測 `incremental.sh:199-202`】 — ラウンド開始**前**に退避しないと delta が常に空になり、
   **global 由来の再生成集合が黙って消える** (負債 1 と同じ壊れ方を、別の原因で起こす)
7. **`--source-url` が 40 桁 hex か誰も検査していない** — 決定 1 の落とし穴 (`coverage.ts` の revless 正規化は
   `/blob/[0-9a-f]{40}/` 決め打ちで、タグ名を渡すと**採点が静かに下がる**)。プロトタイプはこの前提を
   `run.sh:55` / `incremental.sh:106` の**ハードコード既定値に隠している**。`render` / `site` は
   「空でない文字列」しか要求しない【判断】 — 検査を足すのは移設ではなく設計改良なので M3-d2 / M4 が引き取る
8. **`state` を誰が書くかが未決** — プロトタイプは `run.sh:122-134` で**使い捨てページ木**に `global --state` を
   別途回して seed していた (そのうえで 6 成果物を実ページ木と突き合わせ、seed 自体をオラクル化していた)。
   `site` は `--state` を受けるだけ。**増分が「前回のサイトに state がある」前提を置くなら、フル生成側が書く形に決める**
