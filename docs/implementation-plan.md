...
| **M6** | CI テンプレ + README + インストール手順 | **完遂** — `tools/ci-build.sh` を第 2 の対象で実走 (V5)。**GitHub Actions 自体は未実行** → [結果ログ](milestone-log.md) |
...
| V5 | 第 2 の対象で `lake build` と同じジョブに置いたときの CI 時間 | **M6 で実測** → [結果ログ](milestone-log.md) | — |
