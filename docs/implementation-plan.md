...
| V2 | `contentHash` キャッシュを残る IR 全読みに当てたときの取り分 | M3 の後。**測る道具はできた** — 仕事量カウンタが `lean-doc-build.json` の `work.irReads` に出す【2026-08-16】。**回数はラウンド数で動く** (無変更 1 / 1 ラウンド 4 / 2 ラウンド 5)、**`ownership` は N+1 読みで差分側はキャッシュで消せない** → approach.md §5.6 | 取り分が小さければ入れない (構造は残す) |
