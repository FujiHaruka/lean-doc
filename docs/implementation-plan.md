...
| **M7** | **依存パッケージへのリンクを版固定の GitHub blob URL にする** (approach.md §8「外部リンクの版ズレ」案 b) | 参照木から作ったオラクルに対し、**依存リンクが全数一致**。加えて生成した URL の **HTTP 生存率をサンプルで実測**。**doc-gen4 とのバイト一致は要求しない** (ゲート A は保留 → §1) |
...
| | やること | ゲート |
|---|---|---|
| **M7-a** | `.lidx` に**ソース位置**を載せる (抽出器 `--link-index` + `link_index.rs` のパーサ) | 参照木由来のオラクルに対し、`.lidx` から組んだ URL が全数一致 |
| **M7-b** | 依存パッケージの **url + rev の解決** (manifest / `lean --githash`)。モジュール名 → パッケージの規則 | 15 パッケージ + core で解決漏れ 0 |
| **M7-c** | レンダラの依存リンクを blob URL に**既定で**切り替え | 自パッケージのリンクは相対のまま (変えない)。依存リンクが全数オラクル一致 |
| **M7-d** | **実測** — オラクル一致率と、生成 URL の **HTTP 生存率をサンプルで** | 母数とラベル付きで `benchmarks/results/` に落ち、docs から辿れる |
