...
| **M5** | **第 2 の対象**で動かす (依存写像を生成側で作る経路の実測を含む) | **完遂** (a: V1 成立 / b: 合成の第 2 の対象で doc-gen4 の木ゼロで **19/19**) → §7 |
| **M6** | CI テンプレ + README + インストール手順 | **完遂** — `tools/ci-build.sh` を第 2 の対象で実走 (V5)。**GitHub Actions 自体は未実行** → §7 |
...
**CI ランナーは cold 側なのでこれは下限。** 出たサイト 19 ファイルは同じ IR からの `site` と差分 0。
**workflow YAML は構文検証のみ** (ジョブ 1 / ステップ 8)。**Actions では一度も走らせていない。**
...
| V5 | 第 2 の対象で `lake build` と同じジョブに置いたときの CI 時間 | **M6 で実測** → §7 | — |
