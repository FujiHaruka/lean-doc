...
| ゲート | 完了条件 | 判定 |
|---|---|---|
| **A: 移設** | Rust 版が対象リポジトリで **439/439 byte 一致** | `coverage.ts` (Deno、**製品外に残す**)。機械的。**M7 で保留 → 下の枠** |
...
> **ゲート A は M7 で保留に入った**【決定 2026-08-15、ユーザー判断】。M7 が外部リンクを版固定の
> GitHub blob URL に切り替えると、**出力は doc-gen4 とバイトで一致しなくなる**。**doc-gen4 互換は
> もう追わない** (最終的にスタイルも変える方針)。**ゲート A の再定義はユーザーが後日行う。**
> それまでの扱い:
> - 上の **99.5062%** は**書き換えない**。**M6 時点の到達点**であって、**M7 以降の出力に対する主張では
>   ない** — 書き換えると M1〜M6 の結果ログと食い違う。**引用するときは必ず「M6 時点」と添える。**
> - **`coverage.ts` と `tools/*-gate.sh` は消さない。** 再定義のときに母数を組み替える材料になる。
> - **緑の定義は「`cargo test --workspace` が通ること」に落ちる** — doc-gen4 オラクルに依存していた
>   テストは削除ではなく**自前出力に対する回帰テスト**へ退避させる (M7)。「オラクルを失った」ことが
>   テスト名から読めるようにする。
...
製品ツリー `crates/` は M0 で作り、**M1〜M6 が全部入っている** (→ [結果ログ](milestone-log.md))。`experiments/` は数字の
再現性のため凍結してあるので、製品ツリーは既存を壊さず**別に作った**。**抽出器 (Lean)** は製品でもこのまま
使う (移設ではなく**移動**、M4 で `extractor/` へ)、**外側 (TS + シェル)** は Rust に移設 (**未移設はゼロ**)、
