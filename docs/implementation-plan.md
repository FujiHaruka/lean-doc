...
| **M3** | **増分 4 本 + パイプライン**を Rust へ | 本物の移動と本物の削除を `lake build` ごと回して**フルビルドと byte 一致**。**M3-a (`detect`) / M3-b (`ownership` + `merge`) / M3-c (`impact` + `prune`) / M3-d1 (`site`) / M3-d2 (`incremental` + `modules`) は通過済み** → §7。残りは **M3-d3** (差分ハーネス) と **M3-d4** (クローンでのゲート) |
...
**IR は 4 状態で全件一致、3 状態 (added / restored / stale-state) で `index.json` だけ差分**【新しい実測】 —
`merge` は base index に無いモジュールを**末尾に push** し (`merge.rs:334`)、from-scratch 抽出は
モジュール一覧の順に並べるので、**モジュールが増えた木は同じエントリを違う順で持つ**。
entries の集合と他の全キーが一致することをテストが assert し、差分の出る状態集合を固定してある。
**サイトのバイトには届かない**【実測。7 状態すべてで site 一致】。M3-b の 436/436 はモジュールの増減が
無い合流だったので、これは M3-d2 で初めて見えた。
