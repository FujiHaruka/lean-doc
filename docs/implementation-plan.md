...
3. **heading id の分割表** — `render.ts:1058` は **V8** の `\p{P}\p{Z}\p{C}` で切るが、Rust の
   `heading_id` は doc-gen4 (`DocString.lean:155-165`) と同じ **UnicodeBasic**。**4,802 コード
   ポイントで食い違い、向きは一方向** (UnicodeBasic ⊂ V8)【実測 2026-08-12】。トークン (→ V6) と違い
   heading id は `id=` / `href=#` に出るので**ページのバイトを作る**が、対象パッケージの docstring
   1,634,037 コードポイントに**出現 0 件**なので M1 の全件比較は緑のまま通った【実測】。
   **Rust が doc-gen4 側**。第 2 の対象では出うる (→ V6 と同じ再測定で拾える)
...
### M2-b の結果 — キャッシュ層と全域写像 delta【すべて実測 2026-08-12】

移設元は `stage7h/global.ts` の `--state` 経路 (127-229) と delta (365-421)、移設先は
`lean-doc-global/src/{state,delta}.rs`。**差し替えたのは `facts_for` 1 箇所** — M2-a が置いた継ぎ目が
意図どおり効き、derivation 側は 1 行も動かなかった。

**オラクルが 1 本増えた** — `global-state.json` を**プロトタイプとバイト一致させた**
(TS 841,949 B / Rust 841,947 B、差は `derivation` の 1 文字列 26 B ↔ 24 B のみ)。
`ModuleFacts::tokens` は 6 成果物のどのバイトにも届かないので**全件比較では原理的に見えなかった**が、
state を合わせたことで **432 モジュール分の facts (imports / tactics / decls / instances / tokens) が
丸ごとバイト照合された**。`STATE_DERIVATION` は §6 の `extractKey` / `renderKey` と同じ規律で
**敢えてプロトタイプと違う文字列** — TS が書いた state は Rust から見て異物であるべき。

**7 状態オラクル** (base / rerun / modified / removed / added / restored / **stale-state**) は Rust の
統合テストに移し、合成 IR と実 IR の両方で回す。実 IR の hit/miss は 0/432 → 432/0 → 431/1 →
430/0 → 430/1 → 429/3 → 0/432 で、**全 14 ステップで 6 成果物がバイト一致**。

**delta はプロトタイプと突き合わせた** — 参照 `name-map.json` に「移動 3 + before だけ 2 + after だけ 1」を
...
**分岐は 35 中 9 が実データに無い**【実測】 — state の異物 4 形 / `changedSample` の 20 件切り詰め /
affected が空 / BMP 外のモジュール名 / `--print-set` 省略 / V8 だけが区切るコードポイント。
**mutation 8 件のうち 6 成果物の全件バイト比較で捕まったものは 0 件**で、§7 の常設ルールがまた成立した。

### M2 のゲート — サイト 438 中 437 が byte 一致【実測 2026-08-12】

`lean-doc render` と `lean-doc global` を**同じ木に出して**プロトタイプの木と比較した
(参照 = `m1/ref-pages` 432 + `m2/ref-global` 6)。**差分 1 = §5 の登録済み乖離 1 件のみ**で、
両コマンドの書き込みは 1 ファイルも衝突しない。**`w7h/base-pages` は参照に使えない**【実測】 —
`run.sh` の `render()` が `--link-index` を渡していない (→ 決定 4) ので、製品の既定とは別物。
...
**12 シナリオを 1 箇所で定義して両実装に回す**【判断】 — 全域成果物と違い、ここで一致すべきは
ファイルではなく**問いへの答え**なので、問いを両側で書くと誰も意図していない 2 つを比べることになる。

**78 ファイルすべて一致**。台帳 8 本は**意図的に変えた 2 文字列 (計 15 B) だけ**が違い、
鍵名でアンカーして置換すると 1 バイト差なし (`ledger-sha256.json` TS 142,572 / Rust 142,557 B)。
`check` の出力 36 本 (`--changed-out` / `--removed-out` / `--render-all-out`) は**全部バイト一致**。
`--concurrency 8` の台帳は `--concurrency 1` と完全一致 = **バイトはスケジューリングに依存しない**。

**依存パッケージを参照集合に足した**【判断】 — 対象パッケージの 432 モジュールは `.olean` 1 本ずつで、
**module system の 3 ファイル形が構造的に無い**。Mathlib 8 モジュール (24 ファイル / 10,323,184 B) を
同じ台帳形式で照合して埋めた。これで `.olean.private` を落とす変異がバイトで捕まるようになった
— **隣のパッケージは「対象が持たない形」を安く供給する**ので M3-b 以降でも同じ手が使える。

**分岐は 53 中 19 が実データに無い**【実測】。**mutation 7 件のうち、対象パッケージの台帳バイト比較で
捕まるのは 1 件だけ** (モジュールハッシュがファイルパスを無視する変異)。和集合→積集合も、
...
生ログ `benchmarks/results/m3a-ledger-*.jsonl`】 — warm 中央値 Rust **0.1424 s** / TS **0.2468 s**、
cold は 0.2883 / 0.3797 s (各 8 回、run 1 は捨てる)。**分母は同一の仕事**。
**言語の速さの話にはできない** — 内側の `hashSeconds` は両者ともハードウェア SHA-256 に届いていて、
差の出所は読み取り経路 (Deno は 238 MB を JS ヒープに載せ peak RSS 147 MB 対 8.2 MB) と
プロセス起動 (0.005 s 対 0.037 s)。cold−warm の増分は +0.146 / +0.133 s とほぼ等しく、I/O 分は
実装に依存しない。
...
(V8 のみが区切る 4,803 / UnicodeBasic のみ 0、47 区間、U+088F 〜 U+323B0-33479)。全部が
「V8 の UCD では未割当 (`Cn` ⊂ `C`)、UnicodeBasic では割当済み」という **UCD 版差**。
つまり食い違いの 100% が**取りこぼす側**で、計画が想定した危険な向きしか存在しない。

→ **`is_token_separator` = 両表の和**にした。`lean_doc_md::gc` とレンダラは 1 バイトも触らず
(あちらは doc-gen4 に合わせる義務がある)、V8 の表は delta 側の crate に生成物として置いた
(`lean-doc-global/src/v8_gc.rs`、生成器の `--check` が陳腐化を検出)。
**6 成果物 1,877,124 B と state 841,947 B はバイト不動**。

**対象パッケージには 1 件も出現しない**【実測】 — docstring 4,909 件 / 1,634,037 コードポイントに
食い違う 4,803 個は 0 件。「コーパスが届かない」が感想ではなく**測定値**になり、分岐台帳
`tokenSeparatorV8Only` がその事実への依存を明示している。**逆向きの分岐は作っていない**【判断】 —
実測で空集合なので、作れば恒久的に到達不能なエントリになる。代わりに「空である」を assert してある。

**和にしたことは mutation では守れない**【実測】 — UnicodeBasic ⊆ V8 なので「和」と「V8 のみ」は
**等価なプログラム**で、どんなテストでも区別できない (UnicodeBasic のみに落とすと 4 本落ちる)。
第 1 項が冗長でなくなる日は、上の「空である」assert が落ちて告げる。
