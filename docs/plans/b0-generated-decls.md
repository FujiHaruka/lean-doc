...
**状態**: 調査完了、2026-08-21。**B-3 として実装済み**(同日) —— 結果は §15、数字は `benchmarks/results/generated-decls-2026-08-21.txt`。
**§11 の `selectionRange` の説明は結果に否定されたので訂正が入っている。**
...
1. **`(line, col)` に加えて `selectionRange`**(`findDeclarationRanges?` はこちらも返す)。
   ...
   > **【この段落は B-3 が否定した。実測 2026-08-21 →
   > `benchmarks/results/generated-decls-2026-08-21.txt` §1】**
   > 当初ここには「これで**範囲が属性トークンを指している**ことがこの側で判定できる」と
   > 書いてあった。**両方向とも偽である。**
   > **偽陽性**: Mathlib 標本で `selectionRange == range` の **779 件中 209 件 (26.8%)** は
   > 属性トークンではない —— structure / class の field 射影、constructor、
   > `library_note` や `notation` のように 1 つの syntax で位置を記録する macro 定義。
   > 計測対象ではさらに極端で **197 件中 195 件 (99.0%)**。
   > **偽陰性**: 範囲が属性トークンを指しているのに `sel != range` になるものが **376 件**。
   > Mathlib の `to_additive` は `range` に属性全体を、`selectionRange` に
   > **著者が書いた加法版の名前**(書いていなければ `to_additive` トークン)を入れる。
   >
   > **正しい言い方**: `selectionRange` が言うのは「**原典がこの宣言に固有の名前の位置を
   > 与えているか**」だけ。`sel == range` は「実現した elaborator が指せる syntax を
   > 1 つしか持たなかった」ことを意味する。**単独では判定に使えない**が、
   > 「どの宣言を見ているか」を既に知っている規則の**後半**としては正確に効く
   > (下の 2 と合成した 2 条件。実装は `extractor/Extract.lean` の `extOriginOf` 1 箇所)。
...
- **`crates/litedoc4-ir/src/model.rs` の `Decl::index` の doc**:
  ...
  **B-3 で直した。**
- **`crates/litedoc4-render/src/page.rs` の module doc** と
  `model.rs` の `ModuleFile::tactics` の doc の母数 → **B-3 が触る前に既に直っていた**
  【実測 2026-08-21】。CLAUDE.md の「項目を潰す前にまず現況を確認する」がそのまま出た形で、
  **申し送りの方が腐っていた**。

## 15. B-3 の結果(2026-08-21)

**実装した。撤退ラインは踏まなかった。** 数字と条件は
`benchmarks/results/generated-decls-2026-08-21.txt`、計画側の記録は
`docs/plans/feature-sweep.md` §4 B-3 の「実装で決まったこと」。

この文書に対して結果が動かしたのは 2 点:

1. **§11 の `selectionRange` の説明は誤り**だった(上に取り消し線ではなく訂正を入れた)。
2. **§13.2 の反証条件は 3 版で成立した** —— 生成宣言が由来より前に並ぶ件数は
   v4.31.0 / v4.32.2 / v4.33.0 で **0 / 0 / 0**。**§6 は書き換えない。**

動かなかったもの(この文書のまま):§3 / §4 の全 5 規則の否定、§5 の 47 群、
§7 の 190/190、§8 の双子の位置。**B-3 はこれらを再計測していない。**
