...
**このゲートは必ず一度落としてから通す** (CLAUDE.md)。落とし方は決めてある:
`script docs` が渡す `--lib` をわざと空にすると 5 が落ち、Lake 側の extract を
`build.sh` 側と差し替えると 4 が**落ちないこと**が確認できる (= 4 が本当に両経路を
見ているかは、**片方を壊して落ちるか**で確かめる)。
