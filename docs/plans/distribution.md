...
| | 件数 | |
|---|---:|---|
| 配布経路の実装 | **5** | cache restore が自分の出力を上書き / `GITHUB_ACTION_REF` が空 / ref だけで版を選ぶと `@main` が古いバイナリを掴む / `gh` を git 木の外で叩く / 静的リンク検査が `ldd` の文言依存 |
| **検査自身** | **5** | `pipefail` で依存ゼロが死ぬ / 起動していないのを「拒否された」と緑 / lake の解決失敗を toolchain の答えと誤読 / キー名を推測して何も見ていない / 1 回目の描画数がキャッシュ次第 |
| 既に赤かった | **1** | `provenance-gate` が日本語の「派生物」を探していて main が 4 コミット赤 |

**最初から緑だったものは 1 つも無い。** うち **3 件は「緑を返していたが何も測っていなかった」**
形で、CLAUDE.md「品質ゲート」に一般形を足した。
...
> **結果 — 完遂**【2026-08-18】。`v0.1.1` → `v0.1.2` → `v0.1.3` を公開。
