...
— 変えたのは見出しの階層と、下記の 1 件の参照だけ)。**最終更新**: 2026-08-15。
...
> 元から `.gitignore` されていて、tag にも入っていない。**「凍結バイナリとの 436/436
> バイト一致」は、**このマシンのローカルファイルが消えた時点で再現できない**
> — 撤去とは無関係に、M4-a の時点でそうなっていた。
...
| **UI-2 リンク** | → 下の「残った 404」 |
| **UI-3 モバイル** | **未判定。** CSS は 375 px でサイドバーをドロワーに落とし、本文を 1 カラムにし、シグネチャだけを自分の中でスクロールさせるよう書いてある — が、**実ブラウザで描画を見ていない**。「そう書いた」と「そう見える」は別 |
| **UI-4 再ホスト** | M8-e |
