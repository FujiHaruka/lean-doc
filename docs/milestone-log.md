...
| | HEAD (M8-b) | 後 | 差 |
|---|---:|---:|---:|
| raw | 21,516 B | **27,378 B** | +27.2% |
| gzip -9 | 6,529 B | **8,799 B** | +34.8% |
| **規則だけ** (コメントを除く) | 16,310 B | **17,366 B** | **+6.5%** |
| コメント | 5,206 B | 10,012 B | +92.3% |
| 行 | 549 | 656 | +107 |

**ブラウザが実行するぶんは 6.5% しか増えていない。** raw の増分の **82% はコメント**で、
`ui-redesign.md` §9 の撤退ライン (「doc-gen4 の 836 行より大きくなったら削る」) は
**行数 656 でも規則バイト 17,366 B でも下回っている** (doc-gen4 は 18,677 B / 836 行)。
...
罫線が空の箱に蓋をした**。どちらも直した (詳細は plans §10.4)。
**「対象が持たない形を fixture に持たせる」理由がそのまま効いた回。**
